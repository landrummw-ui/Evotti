...
Grain: one row per (workday x dealer x product line). Dealers roll up to
regions (two dealers per region), so every region / product-line total is
unchanged from the old region-grain data -- the sales analysis reads
identically; the dealer dimension is simply added underneath.
...
def _largest_remainder(total, weights):
    """Split integer `total` across `weights` (~sum 1) into whole numbers that
...
def _split_float(total, weights, ndigits=2):
    """Split a float across weights; the last part absorbs rounding so the parts
    sum back to `total`."""
    parts = [round(total * w, ndigits) for w in weights[:-1]]
    parts.append(round(total - sum(parts), ndigits))
    return parts
...
def main():
    ...
    # Separate stream for the dealer split, so the region-cell values (and thus
    # every existing aggregate) are byte-for-byte identical to the old data.
    drng = random.Random(SEED ^ 0x0D9A1E4)
    rows = []
    ...
    for d in workdays(START, END):
        ...
        for region, (rw, pf) in REGIONS.items():
            for line, (lw, asp) in LINES.items():
                ...
                # Split this region cell across its dealers. Actual units by a
                # per-day dealer weight (base share nudged by small noise);
                # actual revenue follows the unit split; the smooth plan splits
                # by fixed base share. Every part sums back to the cell exactly.
                dealers = DEALERS[region]
                shares = [s for (_, s) in dealers]
                w = [max(0.05, s * math.exp(drng.gauss(0, 0.12) - 0.12 * 0.12 / 2))
                     for s in shares]
                sw = sum(w)
                w = [x / sw for x in w]

                au = _largest_remainder(act_units, w)
                fu = _split_float(fc_units, shares)
                if act_units > 0:
                    ar = _largest_remainder(act_rev, [x / act_units for x in au])
                else:
                    ar = _largest_remainder(act_rev, w)
                fr = _largest_remainder(fc_rev, shares)

                for i, (dname, _s) in enumerate(dealers):
                    rows.append({
...
def _write_sql(rows):
    ...
    with open(path, "w") as f:
        ...
        f.write("-- Grain: one row per workday x dealer x product line.\n\n")
        f.write("truncate table sales_daily;\n\n")
        f.write("insert into sales_daily\n")
        f.write("  (sale_date, dealer, region, product_line, units_actual, "
                "units_forecast, revenue_actual, revenue_forecast)\nvalues\n")
        vals = []
        for r in rows:
            vals.append(
                "  ('{sale_date}', '{dealer}', '{region}', '{product_line}', "
